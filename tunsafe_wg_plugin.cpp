...
bool TunsafePluginImpl::OnUnknownPeerSetting(WgPeer *peer, const char *key, const char *value) {
  ...
  // EndpointTCP: separate TCP endpoint for hybrid_tcp handshake.
  // Handled here so it works even with older wireguard_config.cpp builds.
  if (strcmp(key, "EndpointTCP") == 0) {
